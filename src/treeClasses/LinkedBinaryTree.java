	...
	// Creating a CLONE
			...
			private void cloneSubtree(Position<E> rThis, LinkedBinaryTree<E> other, Position<E> rOther) {
				...
				for (Position<E> pThis : children(rThis)) { 
					...
					Node<E> parent = (Node<E>) rThis;
					Node<E> otherParent = (Node<E>) rOther;
					if(parent.getLeft() == pThis){
						otherParent.setLeft(this.createNode(pThis.getElement(), otherParent, null, null));
						cloneSubtree(pThis, other, otherParent.getLeft());
					}
					else{
						otherParent.setRight(this.createNode(pThis.getElement(), otherParent, null, null));
						cloneSubtree(pThis, other, otherParent.getRight());
					}
